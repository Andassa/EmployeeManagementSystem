...
namespace EmployeeManagementSystem
{
    ...
    {
        SqlConnection connect = new SqlConnection(@"Data Source=(LocalDB)\MSSQLLocalDB;AttachDbFilename=C:\Users\Andassa\Documents\employee.mdf;Integrated Security=True;Connect Timeout=30");
                ...
                if(connect.State == ConnectionState.Closed)
                {
                    try
                    {
                        ...
                        using(SqlCommand checkEm = new SqlCommand(checkEmID, connect))
                        {
                            ...
                            if(count >= 1)
                            {
                                MessageBox.Show(addEmployee_id.Text.Trim() + " est déjà pris"
                                    , "Error Message", MessageBoxButtons.OK, MessageBoxIcon.Error);
                            }
                            else
                            {
                                ...
                                string path = Path.Combine(@"C:\Users\Andassa\source\repos\EmployeeManagementSystem\EmployeeManagementSystem\Directory\"
                ...
                if (check == DialogResult.Yes)
                {
                    try
                    {
                        ...
                        using (SqlCommand cmd = new SqlCommand(updateData, connect))
                        {
                            ...
                            MessageBox.Show("Mis à jour avec sucès"
                                , "Information Message", MessageBoxButtons.OK, MessageBoxIcon.Information);
